...
function stInjectCss(){
  ...
  s.textContent =
    ...
    '.st-sub{padding:0 14px;color:#4F4535;font-size:12px}'+
    '.st-gate{margin:12px 14px;padding:12px;background:#fbe7d8;border:1px solid #e3c79a;border-radius:10px}'+
    '.st-gate b{color:#7a1218}'+
    '.st-who{display:flex;align-items:center;justify-content:space-between;gap:8px;margin:12px 14px;font-size:13px;color:#4F4535}'+
    ...
    '.st-label{font-size:11px;color:#4F4535;text-transform:uppercase;letter-spacing:1px;margin-top:2px}'+
    ...
    '.st-muted{font-size:12px;color:#4F4535}'+
    ...
    '.st-prev{margin-top:3px;font-size:11px;color:#4F4535;font-style:italic}'+
    '.st-prevtotal{padding:6px 14px 0;font-size:12px;color:#4F4535}'+
    ...
    '.st-cmp-sub{font-size:12px;color:#4F4535;margin-top:3px;line-height:1.45;max-width:640px}'+
    ...
    '.st-cmp-lbl{font-size:11px;letter-spacing:.8px;text-transform:uppercase;color:#4F4535;margin-top:2px}'+
    ...
    '.st-cmp-count{font-size:12px;color:#4F4535;margin-bottom:8px}'+
    ...
    '.st-cmp-grp{font-size:11px;color:#4F4535;margin-top:2px}'+
    '.st-cmp-note{color:#7a1218}'+
    '.st-cmp-sm{font-size:11px;color:#4F4535}'+
...
function stReportHtml(){
  ...
  return {
    ...
    html: '<div style="font-family:Arial,Helvetica,sans-serif;color:#2a1a10;max-width:680px">'+
      ...
      '<div style="font-size:11px;color:#4F4535;margin-top:14px">Sent from Roberto\'s FOH App · Stock Take</div></div>'
  };
}
...
function stShowUpload(){
  ...
  box.innerHTML='<div class="st-modal-box" onclick="event.stopPropagation()">'+
    ...
    '<div style="font-size:12px;color:#4F4535;margin-bottom:10px">Pick the Excel file the cost controller sent (.xls or .xlsx). It becomes the '+stEsc(stDeptLabel())+' count for the date below.</div>'+
    '<input type="file" id="st-up-file" accept=".xls,.xlsx" style="margin-bottom:10px" onchange="stUploadPreview()">'+
    '<label style="font-size:12px;color:#4F4535">Date counted</label>'+
    '<input id="st-up-month" type="date" style="margin:4px 0 4px">'+
    '<div style="font-size:11px;color:#4F4535;margin-bottom:10px;line-height:1.4">Each date is a separate count kept for good. Counting twice in a month? Use the real date of each one — the earlier count is never touched.</div>'+
...
function stVoiceShowListening(mode){
  ...
  b.innerHTML='<div class="st-modal-box" style="text-align:center" onclick="event.stopPropagation()">'+
    ...
    '<div style="font-size:12px;color:#4F4535;margin-bottom:8px">'+sub+'</div>'+
...
function stVoiceShowConfirm(heard, qty, cands){
  ...
  b.innerHTML='<div class="st-modal-box" onclick="event.stopPropagation()">'+
    ...
    '<div style="font-size:12px;color:#4F4535;margin-bottom:12px">Heard: "'+stEsc(heard)+'"</div>'+
    '<label style="font-size:12px;color:#4F4535">Item'+(cands.length>1?' — '+cands.length+' matches, pick the right one':'')+'</label>'+
    '<select id="st-voice-item" class="st-select" style="width:100%;height:40px;margin:4px 0 12px">'+opts+'</select>'+
    '<label style="font-size:12px;color:#4F4535">Add this many</label>'+
